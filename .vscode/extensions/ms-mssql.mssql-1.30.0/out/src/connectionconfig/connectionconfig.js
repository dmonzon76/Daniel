...
class ConnectionConfig {
    ...
    constructor(_vscodeWrapper) {
        ...
        this.logger = logger_1.Logger.create(this._vscodeWrapper.outputChannel, "ConnectionConfig");
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (const profile of profiles) {
                ...
                if (connectionProfile_1.ConnectionProfile.addIdIfMissing(profile)) {
                    madeChanges = true;
                    this.logger.logDebug(`Adding missing ID to connection '${(0, connectionInfo_1.getConnectionDisplayName)(profile)}'`);
                }
                // ensure each profile is in a group
                if (!profile.groupId) {
                    profile.groupId = rootGroup.id;
                    madeChanges = true;
                    this.logger.logDebug(`Adding missing groupId to connection '${(0, connectionInfo_1.getConnectionDisplayName)(profile)}'`);
                }
            }
            // Save the changes to settings
            if (madeChanges) {
                this.logger.logDebug(`Updates made to connection profiles and groups.  Writing all ${groups.length} group(s) and ${profiles.length} profile(s) to settings.`);
