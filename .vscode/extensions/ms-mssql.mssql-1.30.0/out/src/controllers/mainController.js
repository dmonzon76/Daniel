...
class MainController {
    ...
    initializeObjectExplorer() {
        ...
        if (this.isRichExperiencesEnabled) {
            ...
            this._context.subscriptions.push(vscode.commands.registerCommand(Constants.cmdVisualizeSchema, (node) => __awaiter(this, void 0, void 0, function* () {
                const uri = this.connectionManager.getUriForConnection(node.connectionInfo);
                const schema = yield this.schemaDesignerService.getSchemaModel({
                    connectionUri: uri,
                    databaseName: node.metadata.name,
                });
                console.log(schema);
                const schemaDesignerWebvie = new schemaDesignerWebviewController_1.SchemaDesignerWebviewController(this._context, this._vscodeWrapper, this.schemaDesignerService, node.metadata.name, schema);
